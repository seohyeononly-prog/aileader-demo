...
save();
let session=null,loginRole='student',month=new Date(),adminView='calendar',studentView='calendar';
const emblems=[
['챌린지 스타터','과제 제출 20% 달성','첫 도전을 시작한 실행가','2026-07-08','challenge','★'],['챌린지 모험가','과제 제출 50% 달성','배움의 중간 지점을 힘차게 통과한 모험가','2026-07-22','challenge','◆'],['챌린지 마스터','과제 제출 80% 달성','꾸준함으로 완주를 눈앞에 둔 고수','','challenge','♛'],['챌린지 MVP','과제 제출 100% 달성','모든 과제를 완주한 최고의 도전자','','challenge','★','mvp'],
['1단위 출석 요정','1단위 개근','첫 단위를 빠짐없이 지킨 성실 요정','2026-07-12','attendance','✦'],['2단위 출석 기사','2단위 개근','두 단위 동안 출석을 수호한 기사','2026-07-26','attendance','♜'],['3단위 출석 대장','3단위 개근','세 단위를 흔들림 없이 지킨 대장','','attendance','♟'],['4단위 출석 전설','4단위 개근','네 단위 모두 자리를 빛낸 전설','','attendance','♚'],['풀타임 퍼펙트','총기간 개근','과정 전체를 한 번도 빠지지 않은 완주자','','attendance','✓'],
['질문 새싹','첫 질문 등록','호기심을 행동으로 옮긴 첫걸음','2026-07-09','question','?'],['질문 탐험가','질문 5회','궁금증의 지도를 넓혀가는 탐험가','2026-07-24','question','⌕'],['질문왕','질문 10회','배움의 깊이를 더하는 질문의 달인','','question','?'],['인사이트 헌터','운영진이 좋은 질문으로 선정','모두의 시야를 넓힌 날카로운 질문자','','question','!'],
['피드백 수신 완료','첫 회고 제출','자신을 돌아보고 성장 신호를 받은 학습자','2026-07-11','reflection','↙'],['피드백 연금술사','피드백 반영 3회','조언을 실제 변화로 바꾼 연금술사','2026-08-02','reflection','✣'],['성장 기록가','회고 4회','성장의 순간을 꾸준히 기록한 사람','','reflection','✎'],['레벨업 레전드','모든 필수 회고 완료','과정의 변화를 온전히 자기 것으로 만든 전설','','reflection','↑'],
['첫 하이파이브','동료 응원 1회','동료에게 첫 응원의 손길을 건넨 사람','2026-07-10','together','✋'],['다정한 댓글러','동료 응원 5회','따뜻한 말로 학습 분위기를 밝힌 사람','2026-07-30','together','♥'],['팀 텐션 메이커','동료 응원 10회','팀의 에너지를 끌어올린 분위기 메이커','','together','⚡'],['투게더 MVP','운영진 수동 선정','협업으로 팀의 성과를 높인 핵심 동료','','together','★'],
['아이디어 불꽃','첫 아이디어 제안','새로운 가능성에 첫 불꽃을 붙인 기획자','2026-07-15','independent','✦'],['문제 탐정','개선 문제 1건 발견','숨어 있던 문제의 단서를 찾아낸 탐정','','independent','⌕'],['솔루션 건축가','제안한 해결책 채택','실행 가능한 해법을 설계한 건축가','','independent','⌂'],['마감 수호대','과제 3회 연속 기한 내 제출','마감선을 든든하게 지킨 수호자','2026-08-05','independent','◷'],['얼리버드 로켓','마감 24시간 전 제출','누구보다 빠르게 준비를 끝낸 얼리버드','','independent','↑'],['재시동 히어로','미제출 후 다음 과제 정상 제출','멈춤 뒤 다시 힘차게 출발한 히어로','','independent','↻'],['설문 스프린터','설문 3회 연속 공개 당일 제출','설문을 발견하자마자 완주한 스프린터','','independent','»'],['숨은 공로자','운영진 수동 부여','보이지 않는 곳에서 과정을 빛낸 기여자','','independent','◇'],['AI리더 별자리','운영진 최종 선정','배움·실행·협업을 고루 보여준 과정의 빛나는 별','','independent','✧']
].map(function(x,i){return{id:i+1,name:x[0],condition:x[1],description:x[2],date:x[3],group:x[4],icon:x[5],featured:x[6]}});
...
function renderStudent(){
  ...
  const navigation='<nav class="student-nav" aria-label="수강생 메뉴"><button data-student-view="calendar" class="'+(studentView==='calendar'?'active':'')+'"><span>▦</span>캘린더</button><button data-student-view="emblems" class="'+(studentView==='emblems'?'active':'')+'"><span>✦</span>앰블럼</button></nav>';
  const calendarContent='<section class="student-hero"><div><p class="hello">HELLO, '+esc(student.name)+'</p><h1>이번 달 설문 일정을 확인하세요.</h1></div><div class="progress-card"><div class="row"><span>전체 제출 현황</span><strong>'+done+' / '+surveys.length+'</strong></div><div class="progress-bar"><i style="width:'+(surveys.length?done/surveys.length*100:0)+'%"></i></div></div></section><section class="panel calendar-panel"><div class="calendar-head"><div class="month-nav"><button class="icon-btn" data-month="-1">‹</button><h2>'+labelMonth(month)+'</h2><button class="icon-btn" data-month="1">›</button></div><div class="legend"><span><i class="dot"></i>제출 전</span><span><i class="dot done"></i>제출 완료</span><span><i class="dot due"></i>마감 지남</span></div></div>'+calendarHtml(surveys,student)+'</section>';
  app.innerHTML='<div class="student-layout"><header class="student-header"><div class="brand"><span class="brand-mark">S</span>SurveyFlow</div><div class="student-user"><div class="info"><strong>'+esc(student.name)+'</strong><small>'+esc(student.cohort)+'</small></div><span class="avatar">'+esc(student.name.slice(-1))+'</span><button class="logout" data-logout style="color:#8a8f9f">로그아웃</button></div></header><div class="student-shell">'+navigation+'<main class="workspace">'+(studentView==='calendar'?calendarContent:emblemsHtml())+'</main></div></div>';
  app.querySelector('[data-logout]').onclick=function(){session=null;renderLogin()};
  app.querySelectorAll('[data-student-view]').forEach(function(btn){btn.onclick=function(){studentView=btn.dataset.studentView;renderStudent()}});
  ...
  app.querySelectorAll('[data-emblem]').forEach(function(btn){btn.onclick=function(){openEmblem(Number(btn.dataset.emblem))}});
}
function emblemCards(items){return items.map(function(item){return'<button class="emblem-card '+(!item.date?'locked ':'')+(item.featured?'featured':'')+'" data-emblem="'+item.id+'"><span class="emblem-badge"><i>'+item.icon+'</i></span><strong>'+esc(item.name)+'</strong><small>'+esc(item.condition)+'</small></button>'}).join('')}
function emblemsHtml(){const groups=[['challenge','챌린지'],['attendance','출석'],['question','질문'],['reflection','회고와 성장'],['together','동료 응원']];return'<section class="emblem-hero"><p class="hello">MY EMBLEMS</p><h1>나의 앰블럼</h1><p>배움과 실행의 순간을 앰블럼으로 확인해 보세요.</p></section><div class="emblem-sections">'+groups.map(function(g){return'<section class="emblem-section"><h2>'+g[1]+' <small>단계형</small></h2><div class="emblem-grid">'+emblemCards(emblems.filter(function(x){return x.group===g[0]}))+'</div></section>'}).join('')+'<section class="emblem-section"><h2>특별 앰블럼 <small>독립형</small></h2><div class="emblem-grid">'+emblemCards(emblems.filter(function(x){return x.group==='independent'}))+'</div></section></div>'}
function openEmblem(id){const item=emblems.find(function(x){return x.id===id});if(!item)return;const back=modal('<div class="emblem-detail"><button class="close-btn" data-close>×</button><span class="emblem-badge '+(item.featured?'featured':'')+'"><i>'+item.icon+'</i></span><p class="emblem-state">'+(item.date?'획득한 앰블럼':'미획득 앰블럼')+'</p><h2>'+esc(item.name)+'</h2><p class="emblem-description">'+esc(item.description)+'</p><dl><div><dt>획득 조건</dt><dd>'+esc(item.condition)+'</dd></div><div><dt>획득일</dt><dd>'+(item.date?esc(item.date):'아직 획득하지 않았어요')+'</dd></div></dl></div>','emblem-modal');back.querySelector('[data-close]').onclick=function(){back.remove()}}
